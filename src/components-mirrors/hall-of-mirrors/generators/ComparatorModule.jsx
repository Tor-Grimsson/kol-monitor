...
const MODES = ['GT', 'LT', 'EQ', 'NEQ']

export default function ComparatorModule({ id = 'cmp1', label = 'CMP', config, onChange, busRef }) {
  const { inputAExpr = '', inputBExpr = '', mode = 'GT', enabled = false } = config || {}
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const a = aValRef.current
      const b = bValRef.current
      let result = false

      if (mode === 'GT') result = a > b
      else if (mode === 'LT') result = a < b
      else if (mode === 'EQ') result = Math.abs(a - b) <= 5
      else if (mode === 'NEQ') result = Math.abs(a - b) > 5

      const output = result ? 100 : 0
