...
export default function CvKnob({ port, moduleId, active, signalRef, value, onChange, label, min, max, bipolar, variant, labelMinWidth, jackSize = 'sm', direction = 'horizontal' }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: vertical ? 'column' : 'row', alignItems: 'center', gap: 4 }}>
      ...
      <Knob value={value} onChange={onChange} label={label} min={min} max={max} bipolar={bipolar} variant={variant} labelMinWidth={labelMinWidth} />
    </div>
  )
}
