...
function ExprPanel({ expr1, expr2, onExpr1Change, onExpr2Change, enabled, onToggle, id, aConn, aRef, bConn, bRef, clkConn, clkRef, out1Ref, out2Ref }) {
  return (
    <Module label="Expr" enabled={enabled} onToggle={onToggle} u={1}>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center', gap: 6, padding: '0 6px' }}>
        ...
        {/* Row 3: Clk + outputs on the right */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8 }}>
          <LabeledJack type="in" port="clk" moduleId={id} active={clkConn} signalRef={clkRef} label="clk" labelPosition="left" />
...
export default function ExpressionModule({ id = 'expr1', init, preview }) {
  if (preview) return <ExprPanel expr1="wave(t)" expr2="" onExpr1Change={() => {}} onExpr2Change={() => {}} enabled={false} onToggle={() => {}} id={id} aConn={false} aRef={{ current: null }} bConn={false} bRef={{ current: null }} clkConn={false} clkRef={{ current: null }} out1Ref={{ current: null }} out2Ref={{ current: null }} />
  ...
  useModule({
    ...
    process: (inputs, dt, t) => {
      ...
      // Clock rising edge resets t and frame count
  ...
  return <ExprPanel expr1={expr1} expr2={expr2} onExpr1Change={setExpr1} onExpr2Change={setExpr2} enabled={enabled} onToggle={() => setEnabled(!enabled)} id={id} aConn={aConn} aRef={aRef} bConn={bConn} bRef={bRef} clkConn={clkConn} clkRef={clkRef} out1Ref={out1Ref} out2Ref={out2Ref} />
}
