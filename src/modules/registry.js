...
import ClockModule from './control/ClockModule.jsx'
import ClockDividerModule from './control/ClockDividerModule.jsx'
import LFOModule from './control/LFOModule.jsx'
import EnvelopeModule from './control/EnvelopeModule.jsx'
import SequencerModule from './control/SequencerModule.jsx'
import ConstantModule from './control/ConstantModule.jsx'
import LogicModule from './control/LogicModule.jsx'
import ComparatorModule from './control/ComparatorModule.jsx'
import SampleHoldModule from './control/SampleHoldModule.jsx'
import PenModule from './control/PenModule.jsx'
import JoystickModule from './control/JoystickModule.jsx'
import ExpressionModule from './control/ExpressionModule.jsx'
...
import WaveshaperModule from './math/WaveshaperModule.jsx'
import DelayModule from './math/DelayModule.jsx'
import ReverbModule from './math/ReverbModule.jsx'
import MixerModule from './math/MixerModule.jsx'
import MathsModule from './math/MathsModule.jsx'
import TransformModule from './math/TransformModule.jsx'
import KaleidoscopeModule from './math/KaleidoscopeModule.jsx'
import FilterModule from './math/FilterModule.jsx'
import MagnetoModule from './math/MagnetoModule.jsx'
import S2VModule from './math/S2VModule.jsx'
import V2SModule from './math/V2SModule.jsx'

import RGBOscillatorModule from './generators/RGBOscillatorModule.jsx'
import WaveformModule from './generators/WaveformModule.jsx'
import WireframeModule from './generators/WireframeModule.jsx'
import NoiseModule from './generators/NoiseModule.jsx'
import RampModule from './generators/RampModule.jsx'
import SMX3Module from './generators/SMX3Module.jsx'
import LineGenModule from './generators/LineGenModule.jsx'
import RadialGenModule from './generators/RadialGenModule.jsx'
import ModulatorGenModule from './generators/ModulatorGenModule.jsx'
import GeneratorModule from './generators/GenLofiModule.jsx'
import Generator2Module from './generators/GenHifiModule.jsx'
import DitherModule from './generators/DitherModule.jsx'
import SVGModule from './generators/SVGModule.jsx'
import LifeModule from './generators/LifeModule.jsx'
...
import PatchModule from './utility/PatchModule.jsx'
import PowerModule from './utility/PowerModule.jsx'
import PerfModule from './utility/PerfModule.jsx'
...
export const MODULE_DEFS = {
  ...
  magneto:   { component: MagnetoModule,    hp: 28, u: 3, category: 'math',       label: 'Magneto',     description: '4-head video echo / slit-scan / smear processor. Time-based delay buffer (~2s window, FPS-independent). Three effect algorithms: echo (per-head multi-tap with full tape character), slit (vertex Y-band time-slicing approximation), smear (dense uniform 1-frame taps). Selectable palette (70s/neon/mono), head z-order (layer toggle), dry/wet z-layering (order toggle), per-head feedback or global transport (transportMode). Real tap-tempo with long-press mode-switch (splice/hold/quantize). Per-head individual outputs (h1-h4), inverted output (inv), personality scalar (exp).', controls: [
    { type: 'knob', name: 'Dry/Wet', range: '0–100%', description: 'Dry and wet signal levels' },
    { type: 'knob', name: 'Speed/Pitch', range: '0–100', description: 'Tape playback rate (also set via tap-tempo)' },
    ...
    { type: 'knob', name: 'Rec Lvl', range: '0–100', description: 'Recording level — green<60, yellow≥60 (overdrive), red≥90 (max). LED reflects active recording state.' },
    { type: 'knob', name: 'Clr', range: '0–100', description: 'Background hue (HSL)' },
    { type: 'knob', name: 'Age/Crinkle/Wow/Spring', range: '0–100', description: 'Tape degradation controls' },
    { type: 'knob', name: 'Scl/Ofs', range: '0–100', description: 'Composite scale (50=1×) and playhead time offset' },
    { type: 'selector', name: 'Mode', options: 'echo, slit, smear', description: 'Effect algorithm' },
    { type: 'selector', name: 'Palette', options: '70s, neon, mono', description: 'Head colour palette' },
    { type: 'selector', name: 'Heads', options: 'even, triplet, shift', description: 'Delay spacing pattern' },
    { type: 'selector', name: 'Layer', options: '1234, 4321, alt', description: 'Head colour z-order within wet trails' },
    { type: 'selector', name: 'Pan', options: 'LRLR, center, LRRL', description: 'Head rotation direction sign' },
    { type: 'selector', name: 'Order', options: 'wet, mix, dry', description: 'Z-layering of dry vs wet trails' },
    { type: 'toggle', name: 'Head 1–4', description: 'Per-head enable/disable' },
    { type: 'toggle', name: 'Transport', description: 'When ON, head IconButtons fire global INF/REV/RESTART/PAUSE' },
    { type: 'toggle', name: 'Tap', description: 'Short-press: tap-tempo (splice). Long-press: cycle splice/hold/quantize. Yellow LED blinks at tempo, blue when in select mode.' },
    { type: 'input', name: 'in', signal: 'any', description: 'Signal input' },
    ...
    { type: 'input', name: 'dryCV/spdCV/recCV/sclCV/ofsCV/cutCV/wowCV/sprng', signal: 'scalar', description: 'Offset CV for respective knobs' },
    { type: 'input', name: 'rptCV', signal: 'scalar', description: 'Attenuate CV for repeats' },
    { type: 'input', name: 'pause/inf/rev/fwd/tap/shift/recGate', signal: 'scalar', description: 'Transport CV gates (rising-edge triggers)' },
    { type: 'output', name: 'out', signal: 'points', description: 'Combined dry + wet (z-order via Order toggle)' },
    { type: 'output', name: 'wet', signal: 'points', description: 'Wet groups only' },
    { type: 'output', name: 'dry', signal: 'any', description: 'Input passthrough' },
    { type: 'output', name: 'inv', signal: 'points', description: 'Same as out but with all colours negated' },
    { type: 'output', name: 'exp', signal: 'scalar', description: 'Personality scalar — pulses at tap-tempo, scaled by head activity, damped by tape character' },
    { type: 'output', name: 'h1–h4', signal: 'points', description: 'Per-head wet output (geometry + colour)' },
    { type: 'output', name: 'clk1–4', signal: 'scalar', description: 'Per-head clock pulses (free-running at head delay rate)' },
  ] },
  ...
  output:    { component: OutputModule,       hp: 16, u: 3, category: 'display',    label: 'Output',      description: '4-layer compositing display as the final render destination. Inputs a, b, c, and d are layered with background brightness controlled by knob and CV. Pen input styles the rendering. No outputs, making it a terminal module. 128-sample per-channel history for signal monitoring.', controls: [
    ...
    { type: 'knob', name: 'Trails', range: '0–100', description: 'Frame persistence — 0=full clear each frame, 100=infinite trails. Partial-alpha clear-fill for vidicon/howl-around smear aesthetic.' },
  ...
  recorder:  { component: RecorderModule,   hp: 20, u: 3, category: 'display',    label: 'Recorder',    description: 'Video export module with realtime and offline rendering modes. Records patch output to WebM video at 720p to 4K resolution. 7 aspect ratios (1:1, 3:5, 4:5, 9:16, 16:9, 5:3, 5:4) and 30 or 60fps. Offline mode steps frame-by-frame with fixed timing to guarantee smooth output even from heavy patches. 4 compositing inputs with pen and background control.', controls: [
    ...
    { type: 'knob', name: 'Trails', range: '0–100', description: 'Frame persistence — 0=full clear, 100=infinite trails. Same as Output for visual consistency.' },
